  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        if (user?.role === 'admin') {
          const userResponse = await axios.get(`${import.meta.env.VITE_AUTH_SERVICE_URL}/users`);
          if (userResponse.data.success) {
            const usersData = userResponse.data.users;
            setUsers(usersData);
            
            const userStats = usersData.reduce((acc, user) => {
              acc.totalUsers++;
    ...
    <div>
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          {user?.role === 'admin' ? 'Manage users and system settings' : 'Manage words and artifacts'}
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-indigo-500">
            ...
            <div className="text-3xl font-bold text-gray-900">{stats.wordCount}</div>
          </div>
          <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-teal-500">
            <div className="text-sm text-gray-600 mb-1">Total Artifacts</div>
            <div className="text-3xl font-bold text-gray-900">{stats.artifactCount}</div>
          </div>
        </div>
    </div>
  );
