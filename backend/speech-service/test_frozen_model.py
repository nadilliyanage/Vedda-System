"""
Test the frozen-encoder Vedda model and regenerate test_results.json.
Run this after retrain_frozen_encoder.py completes.
"""
...
FROZEN_MODEL_DIR = 'vedda-asr-model/models/whisper-frozen-v2/final'
...
def main():
    log('=' * 60)
    log('[TEST] VEDDA FROZEN-ENCODER MODEL EVALUATION')
    log('=' * 60)

    if not os.path.exists(FROZEN_MODEL_DIR):
        log(f'[ERROR] Model not found: {FROZEN_MODEL_DIR}')
        log('  Run: python retrain_frozen_encoder.py first')
        sys.exit(1)

    log('[LOAD] Loading frozen-encoder Vedda model...')
    ...
    # Use generation config from model (forced Sinhala)
    ...
    for i, (fid, ref_trans) in enumerate(refs.items()):
        ...
        if audio_path is None:
            results.append({'file': f'{fid}.wav', 'status': 'error', 'transcription': '', 'error': 'Not found'})
            errors += 1
            continue
        ...
        try:
            ...
            results.append({'file': f'{fid}.wav', 'status': 'success', 'transcription': pred, 'reference': ref_trans})
        except Exception as e:
            results.append({'file': f'{fid}.wav', 'status': 'error', 'transcription': '', 'error': str(e)})
            errors += 1
