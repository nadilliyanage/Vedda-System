...
def speech_to_text():
    try:
        ...
        logger.info(f"STT request: {language}")
        ...
        try:
            ...
            if language.lower() == 'vedda':
                logger.info("Using Vedda ASR model...")
                ...
                if vedda_service.is_ready:
                    ...
                    if 'error' not in result or result['error'] is None:
                        transcribed_text = result['text'].rstrip('.').strip()
                        logger.info(f"Vedda STT successful: '{transcribed_text}'")
                        return jsonify({
                            'success': True,
                        ...
                        logger.error(f"Vedda ASR error: {result['error']}")
                        return jsonify({
                            'success': False,
                            'error': result['error'],
                            'text': '',
                    ...
                    logger.error("Vedda ASR model not ready")
                    return jsonify({
                        'success': False,
                        'error': 'Vedda ASR model not available',
                        'text': '',
                        'language': 'vedda'
                    }), 503
