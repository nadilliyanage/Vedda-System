...
if [ -d "$SCRIPT_DIR/dictionary-service" ]; then
  cd "$SCRIPT_DIR/dictionary-service" && python3 app.py > "$SCRIPT_DIR/logs/dictionary.log" 2>&1 &
  sleep 3
fi
...
if [ -d "$SCRIPT_DIR/history-service" ]; then
  cd "$SCRIPT_DIR/history-service" && python3 app.py > "$SCRIPT_DIR/logs/history.log" 2>&1 &
  sleep 3
fi
...
if [ -d "$SCRIPT_DIR/translator-service" ]; then
  cd "$SCRIPT_DIR/translator-service" && python3 app.py > "$SCRIPT_DIR/logs/translator.log" 2>&1 &
  sleep 3
fi
...
# Start Auth Service (Port 5011)
echo "Starting Auth Service on port 5011..."
if [ -d "$SCRIPT_DIR/auth-service" ]; then
  cd "$SCRIPT_DIR/auth-service" && npm start > "$SCRIPT_DIR/logs/auth.log" 2>&1 &
  sleep 3
fi
...
if [ -d "$SCRIPT_DIR/learn-service" ]; then
  cd "$SCRIPT_DIR/learn-service" && python3 app.py > "$SCRIPT_DIR/logs/learn.log" 2>&1 &
  sleep 3
fi
